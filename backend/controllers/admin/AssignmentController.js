...
static async getWaitingAssignments(req, res) {
  try {
    ...
    const autoMatchAvailable = waitingItems.filter((i) => i.autoMatchAvailable).length;
...
// GET /api/assignments/reviewers
static async getAvailableReviewers(req, res) {
  try {
    const { domain_id, exclude_user_ids } = req.query;
    ...
    if (domain_id) {
      whereCondition.teacher = {
        user: {
          userdomain: { some: { domain_id: parseInt(domain_id, 10) } },
        },
      };
    }

    // exclude team members / authors by user_id
    if (exclude_user_ids) {
      const ids = exclude_user_ids.split(",").map((id) => parseInt(id, 10));
      whereCondition.teacher = {
        ...(whereCondition.teacher || {}),
        user: {
          ...(whereCondition.teacher?.user || {}),
          user_id: { notIn: ids },
        },
      };
    }
      ...
      include: {
        teacher: {
          select: {
            ...
            user: {
              select: {
                user_id: true, // <-- so we can expose it as teacher_user_id
                name: true,
                email: true,
                userdomain: {
    ...
    const availableReviewers = reviewers.map((reviewer) => ({
      ...
      teacher_user_id: reviewer.teacher?.user?.user_id ?? null, // <-- clients can use this to exclude
      name: reviewer.teacher?.user?.name || "Unknown Reviewer",
      email: reviewer.teacher?.user?.email || "Unknown",
      designation: reviewer.teacher?.designation || "",
      department: reviewer.teacher?.department?.department_name || "Unknown",
      currentWorkload: reviewer.reviewerassignment?.length || 0,
      status: reviewer.status,
      expertise:
        reviewer.teacher?.user?.userdomain?.map((ud) => ud.domain.domain_name) || [],
      domainIds:
        reviewer.teacher?.user?.userdomain?.map((ud) => ud.domain.domain_id) || [],
      matchScore: domain_id
        ? reviewer.teacher?.user?.userdomain?.some((ud) => ud.domain_id === parseInt(domain_id, 10))
          ? 1
          : 0
        : 0,
    }));

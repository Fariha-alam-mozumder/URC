...
const PaperReviewPage = () => {
  ...
  // You could determine if this is a paper or proposal based on the route or data
  // For now, we'll keep it generic
  const isProposal = window.location.pathname.includes('proposal') || 
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center mb-6">
        ...
        <h2 className="text-2xl font-semibold">
          {isProposal ? 'Research Proposal Review' : 'Paper Review'}
        </h2>
      </div>
      ...
      <div className="bg-white p-6 rounded shadow">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold">
            {isProposal ? 'Research ID' : 'Paper ID'}: {paperId}
          </h3>
        ...
        <h3 className="text-lg font-semibold">
          {isProposal ? 'Research Title' : 'Paper Title'}
        </h3>
        <p className="mb-4">
          Example {isProposal ? 'Research' : 'Paper'} Title for {paperId}
        </p>

        <h3 className="text-lg font-semibold">Abstract</h3>
        <p className="text-sm text-gray-600 mb-4">
          Example abstract for {isProposal ? 'research proposal' : 'paper'} {paperId}. You can load real data here later.
        </p>

        <button className="flex items-center gap-2 border border-gray-300 py-2 px-4 rounded hover:bg-gray-100 text-gray-700">
           Download {isProposal ? 'Research Proposal' : 'Paper'} <span className="text-xs">(PDF · 2.4 MB)</span>
        </button>
      </div>
      ...
      <div className="bg-white p-6 rounded shadow">
        ...
        <button className="w-full bg-blue-900 text-white py-2 px-4 rounded hover:bg-gray-700">
           Submit Review
        </button>
      </div>
    </div>

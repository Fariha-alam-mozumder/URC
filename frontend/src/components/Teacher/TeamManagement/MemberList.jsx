...
const MemberList = ({
  ...
  useEffect(() => {
   ...
   // Require a selected team domain to filter correctly.
   if (!teamDomainId) return;

   const params = {
     includeAllDepts: true,         // <-- key change
     teamDomainId,                  // <-- filter by selected team domain
     creatorUserId,                 // optional: exclude self if teacher
   };

   axios.get(resolveApiUrl("/members"), {
     params,
  ...
  if (pickerMode) {
    return (
      <>
        <button
          onClick={() => teamDomainId ? setShowModal(true) : alert("Please choose a team domain first.")}

...
const MemberList = ({
  ...
  title = 'Team Members',
  members: propMembers,  // <-- Add this prop
}) => {
  const pickerMode = typeof onAddMany === 'function';

  // Use propMembers if passed, else fallback to internal state
  const [members, setMembers] = useState(propMembers || defaultMembers);

  // Update members when propMembers changes
  React.useEffect(() => {
    if (propMembers) {
